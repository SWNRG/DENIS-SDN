...
void coral_init(){
   ...
   route_init();
   PRINTF("Route table size ROUTE_CONF_ENTRIES=%d\n",ROUTE_CONF_ENTRIES);

 /*  PRINTF("Adding 0 to route table...\n");  
   linkaddr_t addr;
	addr.u8[0]=0;
	addr.u8[1]=0;
   route_add(&addr, &addr, 0, 0);
	route_print();*/
...
static void recv_uc_callback(struct unicast_conn *c, const linkaddr_t *from){
   ...
   if(LongRangeReceiving > 0){  //LONG RANGE
		PRINTF("LONG Range Unicast Received Neighbour Data: ");
		c_recv++; //STAT increase received control packet 
		char data_buf[packetbuf_datalen()];
		packetbuf_copyto(data_buf);
		PRINTF("[%s]\n",data_buf);
      ...
      if(getsubstr(data_buf,0,1,PTY)){
         if(!strncmp((char *)PTY,"ND",2)){
            ...
            if(dest.u8[0]==0 && dest.u8[1]==0){  //RM0000 REMOVE ALL routing table records
               ...
               PRINTF("New Removed [%d] routes to %u.%u \n", nr, dest.u8[0], dest.u8[1]); 
              // route_print();  // to del when finish debuging
         ...
         else if(!strncmp((char *)PTY,"CR",2)){
            // PAYLOAD: PTY RAD 
            // SIZE(6):  2   2   
            // EXAMPLE: CR  15  
            int rad=0;
            if(getsubstr(data_buf,2,3,buf)) rad = atoi(buf);
 				radio_set_channel('S',rad);
            PRINTF("Changing Short Radio Channel to [%d] \n", rad); 
         } // CR end
      } // end if getsubstr for PTY
   } else{  // SHORT RANGE
      PRINTF("SHORT Range Control Unicast NO Action\n");
   }
